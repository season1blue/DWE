...
STOP_WORDS = {",", ".", "!", ":", "'", "\"", ";", "/", "\\", "Sex", "Name", "Nick", "Occupation", "Birth", "Languages", "Religion", "Alma", "mater", ""}
...
def parse_args():
    ...
    # Path
    parser.add_argument("--path_qid2abs", default="../data/prepro_data/qid2abs.json", help="Path to id2abs.json")
    parser.add_argument("--path_keys_ordered", default="../data/prepro_data/qids_ordered.json", help="Path to qids_ordered.json")
    parser.add_argument("--dir_cache", default="../data/prepro_data/dm/neg_cache", help="Directory to data model prepro")

    # parser.add_argument("--path_dataset_dev", default="../data/text_data/WIKIMEL_dev.json")
    # parser.add_argument("--path_dataset_train", default="../data/text_data/WIKIMEL_train.json")
    # parser.add_argument("--path_dataset_test", default="../data/text_data/WIKIMEL_test.json")
    parser.add_argument("--path_dataset_train", default="../data/prepro_data/train.json")
    parser.add_argument("--path_dataset_test", default="../data/prepro_data/test.json")

    parser.add_argument("--overwrite_cache", default=True, action="store_true", help="Should overwrite?")
...
# input: qid2abs
# output: tfidf_dict.pkl, neg.json, samples_.json

def main():
    args = parse_args()
    random.seed(args.seed)

    qid2abs = json.load(open(args.path_qid2abs,'r', encoding='utf-8'))

    train_data = json.load(open(args.path_dataset_train, encoding='utf-8'))
    test_data = json.load(open(args.path_dataset_test, encoding='utf-8'))
    dataset = dict(test_data, **train_data)

    # keys_ordered = list(set([data['id'] for data in dataset.values()]))
    keys_ordered = sorted(dataset.keys())

    json.dump(keys_ordered, open(args.path_keys_ordered, 'w'), indent=2)  # qid_ordered.json
