...
def parse_args():
    ...
    parser.add_argument("--path_dataset", default="../data/prepro_data/dataset_clear2.json",
                        help="Path to file 'dataset.json'")
    parser.add_argument("--path_prepro", default="../data/prepro_data/dm", help="Path to prepro dm")
...
def search_fuzz():
    print(f"4 Searching candidates with fuzzywuzzy (num: {num_search}).")
    dataset = load_data()

    ne2qid = json.load(open(os.path.join(dataset_path, "ne2qid.json"), encoding='utf-8'))
    path_search_tmp = join(dataset_path, f"search_tmp{num_search}.json")
    ...
    if True or not exists(path_search_tmp):
        ...
        pool = Pool(40)
        search_res = [pool.apply_async(run, (m, ne_list, num_search,)) for m in mentions]
    ...
    json.dump(men2qids, open(os.path.join(dataset_path, f"search_top{num_search}.json"), 'w'), indent=2)
